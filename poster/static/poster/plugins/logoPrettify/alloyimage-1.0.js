function delArray(h) {
	...
	h.sort();
	var t = this.concat([]);
	for (var n = h.length - 1; n >= 0; n--) t = t.slice(0, h[n]).concat(t.slice(h[n] + 1));
	return t
};
try {
	...
	n = {
		...
		require: function(e) {
			...
			n.src = "./js/module/" + e + ".js",
			n.onload = n.onerror = function(e) {
				t.handlerror(e)
			}
		},
	...
	window[e] = function(r, i, s) {
		...
		this.srcImg = "";
			...
			var f = new Image;
			f.onload = function() {
				u.width = parseInt(this.width),
				u.height = parseInt(this.height),
				a.drawImage(this, 0, 0, this.width, this.height)
			},
			f.src = r
		} else {
			var l = i,
			...
			p = r.height,
			d = h / p;
			i || s ? s ? i || (l = c * d) : c = ~~ (l / d) : (l = h, c = p),
			u.width = l,
			u.height = c,
			isNaN(l) ? a.drawImage(r, 0, 0) : t == "ios" ? n.lib.Fix.drawImageIOS(a, r, l, c) : a.drawImage(r, 0, 0, l, c),
			this.srcImg = r
		}
		this.canvas = u,
		this.context = a,
		this.imgData = a.getImageData(0, 0, u.width, u.height),
		this.name = e + "_" + Math.random(),
		...
		v.height = u.height,
		this.ctxCanvas = v,
		this.ctxContext = u.getContext("2d"),
		this.width = this.canvas.width,
		this.height = this.canvas.height,
		this.useWorker = n.useWorker,
		this.readyState = 1,
		this.useWorker && (this.dorsyWorker = n.lib.dorsyWorker(this));
	...
	onmessage = function(e) {
		var t = e.data,
		r;
		t[0] == "act" ? r = n.reflect(t[1], t[2], t[3]) : t[0] == "add" && (r = n.add.apply(n, t[1])),
		postMessage(r)
	},
	window[e].prototype = {
		set width(e) {
		...
		act: function(e, t) {
			var i = [];
			return i = Array.prototype.slice.call(arguments, 1),
			this.useWorker ? (this.dorsyWorker.queue.push(["act", e, i]), r.call(this)) : n.reflect(e, this.imgData, i),
			this
		},
		...
		complileLayers: function(t) {
			if (this.layers.length == 0) this.tempPsLib = {
				...
				for (var r = 0; r < this.layers.length; r++) {
					...
					s = i[0].layers,
					o = i[0];
					s[s.length - 1] && s[s.length - 1][0].type == 1 && (o = s[s.length - 1][0]),
					n.add(o, i[1], i[2], i[3], t)
				}
			}
		},
		show: function(e, t, n) {
			if (!n && this.useWorker) return this.dorsyWorker.queue.push(["show", e, t]),
			this;
			this.complileLayers(t),
			this.context.putImageData(this.tempPsLib.imgData, 0, 0);
			if (e) if (typeof e == "string") {
				var r = document.querySelector(e);
				r.appendChild(this.canvas)
			} else e.appendChild(this.canvas);
			else document.body.appendChild(this.canvas);
			return this
		},
		replaceChild: function(e) {
			var t;
			return typeof e == "string" ? t = document.querySelector(e) : t = e,
			t.innerHTML = "",
			this.show(t)
		},
		...
		add: function() {
			...
			f = f || "RGB",
			t = arguments[0],
			this.useWorker ? (this.dorsyWorker.queue.push(["add", t, i, s, o, u, a, f]), r.call(this)) : n.add(this.imgData, t.imgData, i, s, o, u, a, f),
			this
		},
		addLayer: function(e, t, n, r) {
			return this.layers.push([e, t, n, r]),
			this
		},
		clone: function(t) {
			var n = new window[e](this.canvas.width, this.canvas.height);
			return n.context.putImageData(this.imgData, 0, 0),
			n.imgData = n.context.getImageData(0, 0, this.canvas.width, this.canvas.height),
			n
		},
		swap: function(e, t) {
			var n = this.layers[e];
			return this.layers[e] = this.layers[t],
			this.layers[t] = n,
			this
		},
		deleteLayers: function(e) {
			this.layers = delArray(e)
		},
		save: function(t, n, i) {
			t = t || "png",
			t = t.toLowerCase(),
			...
			t == "jpg" && (t = "jpeg");
			var s = "image/" + t;
			if (!i && this.useWorker) return this.dorsyWorker.queue.push(["save"]),
			r.call(this),
			this;
			if (!this.layers.length) return this.context.putImageData(this.imgData, 0, 0),
			this.canvas.toDataURL(s, n);
			var o = new window[e](this.canvas.width, this.canvas.height);
			o.add(this, "\u6b63\u5e38", 0, 0, isFast),
			this.tempPsLib = o;
			for (var u = 0; u < this.layers.length; u++) {
				var a = this.layers[u],
				f = a[0].layers,
				l = a[0];
				f[f.length - 1] && f[f.length - 1][0].type == 1 && (l = f[f.length - 1][0]),
				o.add(l, a[1], a[2], a[3], isFast)
			}
			return this.context.clearRect(0, 0, this.canvas.width, this.canvas.height),
			this.context.putImageData(o.imgData, 0, 0),
			this.canvas.toDataURL(s, n)
		},
		saveFile: function(e, t) {
			e = e || "AlloyImage\u5408\u6210\u56fe\u50cf.jpg",
			t = t || 1;
		...
		drawRect: function(e, t) {
			...
			i = [r.indexOf("0") > -1, r.indexOf("1") > -1, r.indexOf("2") > -1]; (n = document.getElementById("imgRect")) || (n = document.createElement("canvas"), n.id = "imgRect", document.body.appendChild(n), n.width = parseInt(this.canvas.width), n.height = parseInt(this.canvas.height));
			var s = n.getContext("2d");
			s.clearRect(0, 0, n.width, n.height);
			var o = [[], [], []];
			this.complileLayers();
			var u = this.tempPsLib.imgData.data,
			a = this.tempPsLib.imgData.width,
			f = this.tempPsLib.imgData.height;
			if (r) for (var l = 0; l < f; l++) for (var c = 0; c < a; c++) {
				var h = l * a + c,
				p = h * 4;
				for (var d = 0; d < 3; d++) i[d] && (o[d][u[p + d]] ? o[d][u[p + d]]++:o[d][u[p + d]] = 1)
			} else for (var h = 0,
			v = u.length; h < v; h++) o[u[h]] ? o[u[h]]++:o[u[h]] = 1;
			var m = function(e, t) {
				s.beginPath(),
				s.moveTo(0, n.height);
				var r = 0;
				...
				for (var i = 0; i < 255; i++) {
					var o = e[i] || 0;
					o = n.height - o / r * .8 * n.height,
					s.lineTo(i / 256 * n.width, o)
				}
				s.lineTo(n.width + 10, n.height),
				s.closePath(),
				s.fillStyle = t,
				s.fill()
			};
		...
		ctx: function(e) {
			...
			return t.putImageData(this.imgData, 0, 0),
			e.call(t),
			this.imgData = t.getImageData(0, 0, this.canvas.width, this.canvas.height),
			this
		},
		notify: function(e) {
		...
		transform: function(t, n, r) {
			...
			v = ~~ (c - h),
			m = ~~ (p - d);
			return o.canvas.width = v,
			o.canvas.height = m,
			o.translate( - h, -d),
			o.transform.apply(o, t),
			o.drawImage(s.canvas, 0, 0),
			this.canvas.width = v,
			this.canvas.height = m,
			this.width = v,
			this.height = m,
			this.imgData = o.getImageData(0, 0, v, m),
			this
		},
		scale: function(e, t) {
			var t = t || e;
		...
		clip: function(e, t, n, r) {
			return this.ctxContext.putImageData(this.imgData, 0, 0),
			this.imgData = this.ctxContext.getImageData(e, t, n, r),
			this.context.canvas.width = n,
			this.context.canvas.height = r,
			this
		},
		Tools: function(e) {
	...
	function(e) {
		var t = {
			FFT1: function(e) {
				...
				function u(r, s, o) {
					...
					f = 0; a <= s - u; a++) {
						...
						c = f * n / Math.pow(2, o),
						h = c + n / 4;
						e[a] instanceof t.C || (e[a] = new t.C(e[a])),
						e[l] instanceof t.C || (e[l] = new t.C(e[l]));
			...
			applyMatrix: function(n, r, i) {
				...
				for (var h = 0,
				p = s.length; h < p; h++) l[h] && (s[h] = l[h] < i ? l[h] : s[h]);
				return n
			},
			RGBToHSI: function(e, t, n) {
				...
				var o = (e + t + n) / 3;
				return i > 2 * Math.PI && (i = 2 * Math.PI),
				i < 0 && (i = 0),
				{
					H: i,
					S: s,
		...
		t.C.prototype = {
			...
			mutiply: function(e) {
				var n = new t.C;
				return n.r = this.r * e.r - this.i * e.i,
				n.i = this.r * e.i + this.i * e.r,
				n
			},
			divide: function(e) {
				var n = new t.C,
		...
		var t = {
			getColor: {
				process: function(t, n) {
					...
					v, m = 0,
					g;
					for (var s = 0; s < l[p].length; s++) d += l[p][s][0],
					m += l[p][s][1];
					v = d / l[p].length,
					g = m / l[p].length;
					var y = p * a,
...
function(e) {
	window[e].module("Alteration.brightness",
	function(e) {
		var t = {
			process: function(e, t) {
				...
				for (var u = 0,
				a = n.length; u < a; u += 4) for (var f = 0; f < 3; f++) n[u + f] = (n[u + f] - 127.5 * (1 - r)) * o + 127.5 * (1 + r);
				return e
			}
		};
		return t
...
function(e) {
	window[e].module("Alteration.curve",
	function(e) {
		var t = {
			process: function(t, n) {
...
function(e) {
	window[e].module("Alteration.gamma",
	function(e) {
		var t = {
			process: function(t, n) {
				var r = e.lib.dorsyMath,
				...
				var a = (u + 100) / 200 * 2;
				for (var f = 0; f < s; f++) for (var l = 0; l < o; l++) r.xyCal(t, f, l,
				function(e, t, n) {
...
function(e) {
	window[e].module("Alteration.selectiveColor",
	function(e) {
		var t = {
			process: function(t, n) {
				var r = n[0],
				...
				v = [i, s, o, u];
				for (var m = 0,
				g = t.width; m < g; m++) for (var y = 0,
				b = t.height; y < b; y++) e.lib.dorsyMath.xyCal(t, m, y,
				function(e, t, n) {
					var i = {
						R: e,
						G: t,
...
function(e) {
	window[e].module("Alteration.setHSI",
	function(e) {
		var t = {
			process: function(t, n) {
				n[0] = n[0] / 180 * Math.PI,
				...
				return e.lib.dorsyMath.applyInHSI(t,
				function(e, t) {
					if (!s[t]) return;
...
function(e) {
	window[e].module("Filter.corrode",
	function(e) {
		var t = {
			process: function(e, t) {
...
function(e) {
	window[e].module("Filter.darkCorner",
	function(e) {
		var t = {
			process: function(t, n) {
				function v(t, n, r) {
	...
	function(e) {
		var t = {
			process: function(t, n) {
				...
				for (var c = 0,
				d = parseInt(o / a); c < d; c++) for (var h = 0,
				v = parseInt(u / a); h < v; h++) {
...
function(e) {
	window[e].module("Filter.embossment",
	function(e) {
		var t = {
			process: function(e, t) {
				var n = e.data,
...
function(e) {
	window[e].module("Filter.gaussBlur",
	function(e) {
		var t = {
			process: function(e, t, n) {
				...
				t = Math.floor(t) || 3,
				n = n || t / 3,
				p = 1 / (Math.sqrt(2 * Math.PI) * n),
				h = -1 / (2 * n * n);
				for (d = 0, a = -t; a <= t; a++, d++) c = p * Math.exp(h * a * a),
				o[d] = c,
				u += c;
				...
				for (f = 0; f < s; f++) for (a = 0; a < i; a++) {
					...
					u = 0;
					for (v = -t; v <= t; v++) m = a + v,
					m >= 0 && m < i && (d = (f * i + m) * 4, l += r[d] * o[v + t], c += r[d + 1] * o[v + t], h += r[d + 2] * o[v + t], u += o[v + t]);
					d = (f * i + a) * 4,
					r[d] = l / u,
					r[d + 1] = c / u,
					r[d + 2] = h / u
				...
				for (a = 0; a < i; a++) for (f = 0; f < s; f++) {
					...
					u = 0;
					for (v = -t; v <= t; v++) m = f + v,
					m >= 0 && m < s && (d = (m * i + a) * 4, l += r[d] * o[v + t], c += r[d + 1] * o[v + t], h += r[d + 2] * o[v + t], u += o[v + t]);
					d = (f * i + a) * 4,
					r[d] = l / u,
					r[d + 1] = c / u,
					r[d + 2] = h / u
...
function(e) {
	window[e].module("Filter.ImageEnhance",
	function(e) {
		var t = {
			process: function(e, t, n) {
				function f(e) {}
...
function(e) {
	window[e].module("Filter.borderline",
	function(e) {
		var t = {
			process: function(t, n) {
	...
	function(e) {
		var t = {
			process: function(e, t) {
				...
				o = n * 2 + 1;
				for (var u = 0,
				a = parseInt(i / o); u < a; u++) for (var f = 0,
				l = parseInt(s / o); f < l; f++) {
					var c = [],
					h = [0, 0, 0];
...
function(e) {
	window[e].module("Filter.oilPainting",
	function(e) {
		var t = {
			process: function(e, t) {
...
function(e) {
	window[e].module("Filter.posterize",
	function(e) {
		var t = {
			process: function(t, n) {
				var r = e.lib.dorsyMath,
	...
	function(e) {
		var t = {
			process: function(t) {
				...
				i = t.width,
				s = t.height;
				for (var o = 0; o < i; o++) for (var u = 0; u < s; u++) n.xyCal(t, o, u,
				function(e, t, n) {
...
function(e) {
	window[e].module("Filter.toReverse",
	function(e) {
		var t = {
			process: function(e) {
				var t = e.data;
				for (var n = 0,
				r = t.length; n < r; n += 4) t[n] = 255 - t[n],
				t[n + 1] = 255 - t[n + 1],
				t[n + 2] = 255 - t[n + 2];
				return e.data = t,
				e
...
function(e) {
	window[e].module("Filter.toThresh",
	function(e) {
		var t = {
			process: function(t, n) {
				...
				n = n[0] || 128;
				for (var i = 0,
				s = r.length; i < s; i++)(i + 1) % 4 && (r[i] = r[i] > n ? 255 : 0);
				return t.data = r,
				t
			}
		};
