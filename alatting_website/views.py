...
class PosterView(DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        if obj.lifetime_type == 'weekly':
            weekly_hours = json.loads(obj.lifetime_value)
            # the lifetime value of hours must be json format, auto generated by program
            hours = weekly_hours[weekday]
            if hours['enabled']:
                if hours['time_start'] and hours['time_end']:
                    time_start = datetime.datetime.strptime(day_now+' '+hours['time_start'],'%Y-%m-%d %H:%M:%S')
                    time_end = datetime.datetime.strptime(day_now+' '+hours['time_end'],'%Y-%m-%d %H:%M:%S')
