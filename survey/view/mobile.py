...
# no questionnaire for current category
		...
		if prev_run:
# check if current user has submitted part of current questionnaire
...
#check if current consumer is the poster creator
		consumer_is_poster_creator = False
		if self.request.user.pk == runinfo.poster.creator.pk and questionnaire.role=="consumer":
			consumer_is_poster_creator = True
#check if current consumer repeatedly submit to the poster creator
...
#check if current consumer has reached to the last page of questionnaire
...
#if current consumer is the poster creator, change title, and diable all inputs
		if consumer_is_poster_creator:
			title = "请浏览您海报上面的调查问卷"
		else:
			title = '请输入详细信息'
		contextdict = {'title': title,
						...
						'consumer_is_poster_creator': consumer_is_poster_creator}
